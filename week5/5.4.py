...
class strategy1(base_strategy):
    ...
    def predict(self,price):
        if self.data[price['date']] == 0:
            ...
            return self.status
        elif self.data[price['date']] % 4 == 0 and self.status == DIRECTION.BUY:
            self.status = DIRECTION.SELL
            return self.status
        elif self.data[price['date']] % 4 == 0 and self.status == DIRECTION.SELL:
            self.status = DIRECTION.BUY
            return self.status
...
class strategy2(base_strategy):
    ...
    def predict(self,price):
        ...
        if sum(self.mavg)/len(self.mavg) >560 and not self.buy:
            self.buy = True
            return DIRECTION.SELL
        elif sum(self.mavg)/len(self.mavg) <559 and self.buy:
            self.buy = False
            return DIRECTION.BUY
#%%
class strategy3(base_strategy):
    ...
    def fit(self,price_update):
        if len(self.historical_data.index)>500:
            ...
            glm.fit(X = self.historical_data[['OpenClose','HighLow']],y=self.historical_data['Trade'])
            return glm
...
if __name__ == '__main__':
    ...
    test_func()
    data = open('/Users/meredithma/Downloads/input000 (1).txt')
    da = data.readlines()
    da = da[2:]
    nb_of_rows = 600 
